 ...
 * catching it internally - callers that want the previous behaviour of silently falling back to the un-reordered
 * system field order should catch it and return {@code systemOrderContent} themselves.
 */
public final class MarcJsonFieldOrderer {
  ...
  public static String reorderFields(String sourceOrderContent, String systemOrderContent) {
    try {
      ...
      prependSystemFieldsFirst(nodes00X, reorderedFields);
      appendFieldsInSourceOrder(nodes, nodes00X, sourceOrderTags, reorderedFields);
  ...
  private static List<String> getSourceFields(String source) {
    ...
    try {
      ...
      classifyFieldsByTag(fieldsNode, sourceFields);
    } catch (Exception e) {
  ...
  private static void classifyFieldsByTag(JsonNode fieldsNode, List<String> sourceFields) {
